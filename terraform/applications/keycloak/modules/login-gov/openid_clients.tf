...
resource "keycloak_openid_client" "my-test-react-app" {
  count                                      = var.enable_my-test-react-app ? 1 : 0
  access_type                                = "PUBLIC"
  backchannel_logout_revoke_offline_sessions = "false"
  backchannel_logout_session_required        = "false"
  client_authenticator_type                  = "xx-client-secret"
  client_id                                  = "my-test-react-app"
  consent_required                           = "false"
  direct_access_grants_enabled               = "true"
  display_on_consent_screen                  = "false"
  enabled                                    = "true"
  ...
  standard_flow_enabled                      = "true"
  use_refresh_tokens                         = "true"
  valid_redirect_uris                        = ["https://pst-amt-test-app-dev.vfs.va.gov/*"]
  web_origins                                = ["*"]
}
